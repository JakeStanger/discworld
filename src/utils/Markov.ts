...
class Markov {
  ...
  private static readonly CACHE_DIR = path.join(__dirname, '../', '../', 'markov');
   ...
   */
  public async load() {
    const members = this.client.guilds.get(config.guild).members.filter(m => !m.user.bot).map(m => m.user);
    ...
    for (let member of members) {
      ...
      if (existsSync(path.join(Markov.CACHE_DIR, member.id + ".json"))) this.chains[member.id] = this._readChain(member);
  ...
  private _readChain(member: User): MarkovChain {
    ...
    console.log(kleur.green("Loaded cached chain for " + kleur.cyan(member.username)));

    return chain;
  }

  private async _createChain(member: User): Promise<MarkovChain> {
    console.log(kleur.green("Generating new chain for " + kleur.cyan(member.username)));
    ...
    const chain = new MarkovChain(messages
    ...
    console.log(kleur.green("Generated chain for " + kleur.cyan(member.username)));
